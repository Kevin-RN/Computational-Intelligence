...
class TSPData:
    ...
    def calculate_routes(self, aco):
        print("calculate routes start")
        self.product_to_product = self.build_distance_matrix(aco)
        print("calculate routes done with distance matrix")
        self.start_to_product = self.build_start_to_products(aco)
        print("calculate routes done with start to products")
        self.product_to_end = self.build_products_to_end(aco)
        print("calculate routes done with products to end")
        self.build_distance_lists()
        print("calculate routes done with distance lists")
        return
    ...
    def build_distance_matrix(self, aco):
        ...
        for i in range(number_of_product):
            ...
            print("distance matrix: " + str(i) + "/" + str(number_of_product))
            for j in range(number_of_product):
                start = self.product_locations[i]
                end = self.product_locations[j]
                print("search shortest for distance: " + str(j) + "/" + str(number_of_product))
    ...
    # @return TSP object with uninitialised routes
    @staticmethod
